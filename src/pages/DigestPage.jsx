...
export default function DigestPage() {
  ...
  const thisWeek  = articles.filter(a=>a.publishedAt?.toDate()>=weekStart)
  const topLiked  = [...articles].sort((a,b)=>(b.likes||0)-(a.likes||0)).slice(0,3)
  const byCat     = thisWeek.reduce((acc,a)=>({...acc,[a.category]:[...(acc[a.category]||[]),a]}),{})
            ...
            {thisWeek[0]&&(
              <section className={styles.topStory}>
                ...
                <Link to={`/article/${thisWeek[0].id}`} className={styles.topCard}>
                  {thisWeek[0].coverImage&&<img src={thisWeek[0].coverImage} alt="" className={styles.topImg} loading="lazy"/>}
                  <div className={styles.topBody}>
                    ...
                    {thisWeek[0].summary&&<p>{thisWeek[0].summary}</p>}
                    <div className={styles.byline}>By {thisWeek[0].authorName} · {fmtDate(thisWeek[0].publishedAt)}</div>
                  </div>
                </Link>
              </section>
            )}
                  ...
                  {arts.map(a=>(
                    <Link key={a.id} to={`/article/${a.id}`} className={styles.digestItem}>
                      <div className={styles.digestBody}>
                        <h3>{a.title}</h3>
                        <span>By {a.authorName} · {fmtDate(a.publishedAt)}</span>
                      </div>
                      {a.coverImage&&<img src={a.coverImage} alt="" className={styles.digestThumb} loading="lazy"/>}
                    </Link>
                  ))}
                </div>
              </section>

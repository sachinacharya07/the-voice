                ...
                <Link to={`/article/${hero.id}`} className={styles.heroLeft}>
                  {hero.coverImage?(
                    <div className={styles.heroImg}>
                      <img src={hero.coverImage} alt={hero.title} loading="eager" decoding="async" onLoad={e=>{e.target.style.opacity=1;e.target.style.filter="blur(0)"}} style={{opacity:0,filter:"blur(10px)",transition:"opacity 0.5s ease, filter 0.5s ease"}}/>
                ...
                {midRow.map(a=>(
                  <div key={a.id} className={styles.midCol}>
                    <Link to={`/article/${a.id}`} className={styles.midLink}>
                      {a.coverImage&&<div className={styles.midImg}><img src={a.coverImage} alt="" loading="lazy" decoding="async" onLoad={e=>{e.target.style.opacity=1;e.target.style.filter="blur(0)"}} style={{opacity:0,filter:"blur(8px)",transition:"opacity 0.4s ease, filter 0.4s ease"}}/></div>}
                      <CatBadge cat={a.category}/>
